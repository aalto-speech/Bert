...
class _LogSessionRunHook(tf.train.SessionRunHook):
  ...
  def after_create_session(self, session, coord):
    if FLAGS.use_fp16 or FLAGS.amp:
      ...
      tf.logging.info("  Step samples/sec   MLM Loss  NSP Loss  Loss  Learning-rate  Loss-scaler")
    else:
      print('  Step samples/sec   MLM Loss  NSP Loss  Loss  Learning-rate')
      tf.logging.info('  Step samples/sec   MLM Loss  NSP Loss  Loss  Learning-rate')
    self.elapsed_secs = 0.
    self.count = 0
...
def main(_):
    ...
    bert_config = model_1.BertConfig.from_json_file('bert_config_0607.json')
    input_files = []
    input_files.extend(tf.gfile.Glob('data/kielipankki_clean_train_300_final0607.tfrecord'))
    ...
    model_fn = model_fn_builder(
        ...
        init_checkpoint='/m/triton/scratch/work/jaina5/Bert/FinnishBert_2.0/outputckpoints_0907/model.ckpt-450000',
        learning_rate=5e-5,
        ...
        hvd=None)
    training_hooks = []
    training_hooks.append(_LogSessionRunHook(40,500,-1))
    ...
    if FLAGS.do_train:
        ...
        tf.logging.info("  Batch size = %d",40)
        train_input_fn = input_fn_builder(
            ...
            is_training=True,
            hvd=None )
        estimator.train(input_fn=train_input_fn, hooks=training_hooks, max_steps=500000)
    ...
    output_eval_file = os.path.join('outputckpoints_0907', "eval_results_2.txt")
