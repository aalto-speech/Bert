...
flags.DEFINE_string(
    "input_segments", 'last_segment',
    "The directory which contains the segments of data.")

flags.DEFINE_string(
    "bert_config_file", 'bert_config_0607.json',
...
flags.DEFINE_string(
    "output_dir", 'outputckpoints_0907',
...
MASKED_TOKEN = "[MASK]"
vocab_words=load_vocab('data/vocab_train')
    ...
    for word_loss in result:
     ...
     #                "prob": float(np.exp(-word_loss[0])) })
      ...
      if all_tokens[i] == "[SEP]":
        ...
        result_str+=str(all_ids[sentence_counter])+" "+str(sentence_loss)+"\n"
...
def main(_):
  ...
  index = 0
  all_ids=[]
  with open(FLAGS.input_segments, "r", encoding="utf-8") as reader:
    while True:
  ...
  output_predict_file = os.path.join(FLAGS.output_dir, FLAGS.input_segments)
   #print(list(result))
  parse_result(result, all_tokens,all_ids, output_predict_file)
