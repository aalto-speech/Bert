...
flags.DEFINE_integer("num_accumulation_steps", 1, "Gradient Accumulation: Number of steps before updating gradients")
class _LogSessionRunHook(tf.estimator.SessionRunHook):
  ...
  def after_create_session(self, session, coord):
    ...
    if FLAGS.use_fp16 or FLAGS.amp:
      ...
      tf.compat.v1.logging.info("  Step samples/sec   MLM Loss  NSP Loss  Loss  Learning-rate  Loss-scaler")
    else:
      print('  Step samples/sec   MLM Loss  NSP Loss  Loss  Learning-rate')
      tf.compat.v1.logging.info('  Step samples/sec   MLM Loss  NSP Loss  Loss  Learning-rate')
    self.elapsed_secs = 0.
    self.count = 0
    ...
    if print_step == 1 or print_step % self.display_every == 0:
        ...
        if self.hvd_rank >= 0:
          if FLAGS.use_fp16 or FLAGS.amp:
            ...
            tf.compat.v1.logging.info("%2d :: %6i %11.1f %10.4e %6.3f     %6.4e  %6.4e" %
                  (self.hvd_rank, print_step, img_per_sec, mlm_loss, total_loss, lr, loss_scaler))
          else:
            print('%2d :: %6i %11.1f %10.4e %6.3f     %6.4e' %
                  (self.hvd_rank, print_step, img_per_sec, mlm_loss,total_loss, lr))
            tf.compat.v1.logging.info("%2d :: %6i %11.1f %10.4e %6.3f     %6.4e" %
                  (self.hvd_rank, print_step, img_per_sec, mlm_loss,total_loss, lr))
        else:
          if FLAGS.use_fp16 or FLAGS.amp:
            print('%6i %11.1f %10.4e %6.3f     %6.4e  %6.4e' %
                  (print_step, img_per_sec, mlm_loss, total_loss, lr, loss_scaler))
            tf.compat.v1.logging.info("%6i %11.1f %10.4e %6.3f     %6.4e  %6.4e" %
                  (print_step, img_per_sec, mlm_loss, total_loss, lr, loss_scaler))
          else:
            print('%6i %11.1f %10.4e  %6.3f     %6.4e' %
                  (print_step, img_per_sec, mlm_loss, total_loss, lr))
            print('Step = %6i Throughput = %11.1f MLM Loss = %10.4e Loss = %6.3f Average Loss = %6.3f LR = %6.4e' %
                    (print_step, img_per_sec, mlm_loss, total_loss, avg_loss_step, lr))
            tf.compat.v1.logging.info('Step = %6i Throughput = %11.1f MLM Loss = %10.4e Loss = %6.3f Average Loss = %6.3f LR = %6.4e' %
                    (print_step, img_per_sec, mlm_loss, total_loss, avg_loss_step, lr))
        self.elapsed_secs = 0.
        self.count = 0
        self.avg_loss = 0.0
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps,num_accumulation_steps, use_tpu,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      train_op = optimization_.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps,num_accumulation_steps, use_tpu,hvd)
      ...
      eval_metrics_op = metric_fn(masked_lm_example_loss, masked_lm_log_probs, masked_lm_ids,
          masked_lm_weights)
      output_spec = tf.estimator.EstimatorSpec(
          mode=mode,
          loss=total_loss,
...
def main(_):
    ...
    bert_config = model_1.BertConfig.from_json_file('bert_config_03092020_2.json')
    input_files = []
    input_files.extend(tf.gfile.Glob('kielipankki_clean_train_300_final0207.tfrecord'))
    ...
    run_config = tf.estimator.RunConfig(
        model_dir='outputckpoints_03092020_2',
    ...
    model_fn = model_fn_builder(
        ...
        hvd=None)
    training_hooks = []
    training_hooks.append(_LogSessionRunHook(128,5000,-1))
    estimator = tf.estimator.Estimator(
        model_fn=model_fn,
        config=run_config)
    if FLAGS.do_train:
        tf.compat.v1.logging.info("***** Running training *****")
        tf.compat.v1.logging.info("  Batch size = %d",128)
        train_input_fn = input_fn_builder(
            ...
            is_training=True,
            hvd=None )
        estimator.train(input_fn=train_input_fn, hooks=training_hooks, max_steps=1200000)
    ...
    output_eval_file = os.path.join('outputckpoints_03092020_2', "eval_results.txt")
