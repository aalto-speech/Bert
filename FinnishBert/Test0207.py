  ...
  with tf.gfile.GFile(output_file, "w") as writer:
    ...
    for word_loss in result:
      ...
      if all_tokens[i] == "[CLS]":
        # sentence = {}
        # tokens = []
        # tokens.append({"token":printable_text(all_tokens[i]),"prob": float(np.exp(-word_loss[0]))})
        # sentence_loss = 0.0
        # word_count_per_sent = 0
        i += 1
...
def main(_):
  ...
  predict_examples =[InputExample(1,'kilpailut+ +taminen merkitsisi noin kaksikymmentä henkilö+ +n joukko+ +liikenne+ +toimisto+ +n perusta+ +miseen turun tapaan sanoo rai+ +ni+ +o joka vakuuttaa ettei joukko+ +liikennettä tampereella voi hoitaa ilman keski+ +tetty+ +ä suunnittelu+ +a')]
  ...
  output_predict_file = os.path.join(FLAGS.output_dir, "test_results_test.json")
  #print(list(result))
